...
/*
 * PowerTag reports are written to stdout as InfluxDB line protocol.
 * They are meant to be piped into powertagd-bridge, which parses them and
 * forwards the metrics over MQTT.
 */
...
int main(int argc, char **argv)
{
	...
	int ch;
	while ((ch = getopt_long(argc, argv, "d:qv", long_opts, NULL)) != -1) {
		switch (ch) {
		case 'q':
			verbose = 0;
			break;
